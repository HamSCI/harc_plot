...
def load_hdf5_data(file_path):
    ...
    try:
        ...
        df['datetime'] = pd.to_datetime(df[['year', 'month', 'day', 'hour', 'min', 'sec']].astype(str).agg('-'.join, axis=1))
        df = df[['datetime', 'tfreq', 'pthlen', 'latcen', 'loncen']]
        df.columns = ['date', 'freq', 'dist_km', 'lat', 'long']
        return df
...
def main(run_dct):
    ...
    data_sources = run_dct.get('data_sources', [1, 2])  # Assuming file paths are mapped

    # Define path for saving NetCDF Files
    if output_dir is None:
        tmp = []
        if filter_region is not None:
            tmp.append(f'{filter_region}')
            tmp.append(f'{filter_region_kind}')
        tmp.append(f'{rgc_lim[0]}-{rgc_lim[1]}km')
        tmp.append(f'dx{xb_size_min}min')
        tmp.append(f'dy{yb_size_km}km')
    ...
    for dt in tqdm.tqdm(dates):
        ...
        for ld_inx, load_date in enumerate(load_dates):
            ...
            for data_source_id in data_sources:
                # Here you would map data_source_id to the actual file path (you should define this mapping)
                file_path = f'/data/madrigal/{data_source_id}.h5'  # Replace with actual logic to fetch file paths

                df_temp = load_hdf5_data(file_path)
                if df_temp is not None:
                    dfs.append(df_temp)
            
            # Concatenate the dataframes loaded from all sources for the current load_date
            if dfs:
                df = pd.concat(dfs, ignore_index=True)

        # Proceed with processing and calculations after loading the data
        if len(df) == 0:
            continue
        ...
        # Process the data (just like the original code)
        for band_inx, (band_key, band) in enumerate(band_obj.band_dict.items()):
            if len(df) == 0:
                continue

            frame = df.loc[df["freq"] == band.get('meters')].copy()
        ...
        # Maps - Concatenate all bands into a single DataArray
        for xkey in xkeys:
        ...
        # Time Series - Concatenate all bands into a single DataArray
        for xkey in xkeys:
            for param in params:
