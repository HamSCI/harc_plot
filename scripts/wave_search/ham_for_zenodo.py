...
#        rd['band']               = band_obj.band_dict[rd['band_MHz']]['meters']
    ...
    def load_data(self):
        ...
        ################################################################################ 
        # Stage data for zenodo in CSV format.

        zkeys = []
#        zkeys.append('rpt_key')
#        zkeys.append('tx')
#        zkeys.append('rx')
#        zkeys.append('rpt_mode')
#        zkeys.append('snr')
        zkeys.append('freq')
        zkeys.append('occurred')
        zkeys.append('source')
        zkeys.append('tx_grid')
        zkeys.append('rx_grid')
#        zkeys.append('band')
        zkeys.append('tx_lat')
        zkeys.append('tx_long')
        zkeys.append('rx_lat')
        zkeys.append('rx_long')
        zkeys.append('dist_Km')
#        zkeys.append('tx_mode')
#        zkeys.append('tx_loc_source')
#        zkeys.append('rx_loc_source')
        zkeys.append('md_lat')
        zkeys.append('md_long')
#        zkeys.append('ut_hrs')
        zkeys.append('slt_mid')
#        zkeys.append('band_MHz')
...
#            E: Estimated using prefix
        ...
        tf = dfz['source'] == 1
        dfz.loc[tf,'source'] = 'WSPRNet'

        tf = dfz['source'] == 2
        dfz.loc[tf,'source'] = 'RBN'

        tf = dfz['source'] == 3
        dfz.loc[tf,'source'] = 'PSKReporter'

        dfz = dfz.rename(columns={'occurred':'ut'})

        zen_name = dt.strftime('%Y%m%d') + '.rbn_pskreporter_wspr_data.csv'
        zen_path = os.path.join(h5s_path,zen_name)

        header = """
# This data file contains the amateur radio spot data used in the manuscript:
#   "First Observations of Large Scale Traveling Ionospheric Disturbances Using Automated Amateur Radio Receiving Networks"
#     by Nathaniel A. Frissell W2NAF, Stephen R. Kaeppler AD0AE, Diego F. Sanchez KD2RLM, Gareth W. Perry KD2SAK,
#        William D. Engelke AB4EJ, Philip J. Erickson W1PJE, Anthea J. Coster, J. Michael Ruohoniemi, and Joseph B. H. Baker
#
# Observations are provided courtesy of:
#   1. The Weak Signal Propagation Reporting Network (WSPRNet, https://www.wsprnet.org/)
#        Operated by Bruce Walker W1BW
#
#   2. The Reverse Beacon Network (RBN, http://www.reversebeacon.net/)
#        Operated by Nick Sinanis F5VIH/SV3SJ, David Pascoe KM3T, Mark Glenn K7MJG, Peter Smith N4ZR, Felipe Ceglia PY1NB/CT7ANO, and Dick Williams W3OA
#
#   3. PSKReporter (https://pskreporter.info/)
#        Operated by Philip Gladstone N1DQ
#
# Explanation of Data Columns:
#   freq:       Frequency [kHz]
#   ut:         Time of Observation in Universal Time (UT)
#   source:     Observation Network (WSPRNet, RBN, or PSKReporter)
#   tx_grid:    Transmitter Maidenhead Gridsquare
#   rx_grid:    Receiver Maindenhead Gridsquare
#   tx_lat:     Transmitter Latitude (deg)
#   tx_long:    Transmitter Longitude (deg)
#   rx_lat:     Receiver Latitude (deg)
#   rx_long:    Receiver Longitude (deg)
#   dist_Km:    Ground-Level Great Circle Distance between Transmitter and Receiver (km)
#   md_lat:     Midpoint between Transmitter and Receiver Latitude (deg)
#   md_long:    Midpoint between Transmitter and Receiver Longitude (deg)
#   slt_mid:    Solar Local Time of Midpoint (hr)
#
# This datafile is a subset of the full WSPRNet/RBN/PSKReporter data, filtered with the following parameters:
"""
        l   = []
        l.append(header[:-1])
        l.append('#   ut min:       {!s}'.format(dfz['ut'].min()))
        l.append('#   ut max:       {!s}'.format(dfz['ut'].max()))
        l.append('#   freq min:     {!s}'.format(dfz['freq'].min()))
        l.append('#   freq max:     {!s}'.format(dfz['freq'].max()))
        l.append('#   md_lat min:   {!s}'.format(dfz['md_lat'].min()))
        l.append('#   md_lat max:   {!s}'.format(dfz['md_lat'].max()))
        l.append('#   md_long min:  {!s}'.format(dfz['md_long'].min()))
        l.append('#   md_long max:  {!s}'.format(dfz['md_long'].max()))
        l.append('#   dist_Km min:  {!s}'.format(dfz['dist_Km'].min()))
        l.append('#   dist_Km max:  {!s}'.format(dfz['dist_Km'].max()))
        l.append('#\n')

        header = '\n'.join(l)
        with open(zen_path,'w') as fl:
            fl.write('# {!s}\n'.format(zen_name))
            fl.write('#\n')
            fl.write(header[1:])
        dfz.to_csv(zen_path,index=False,mode='a')
...
#
        ...
        fig.text(0.5,0.98,title,fontdict={'weight':'bold','size':24},ha='center')
...
if __name__ == '__main__':
    output_dir  = os.path.join('output/ham_for_zenodo')
