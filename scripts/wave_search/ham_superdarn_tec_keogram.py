...
#        ax.set_title('(b)',{'size':lbl_size},'left')
        ...
        fit_to_csv = []
        for fitrec in fitacf:
            fit_ut = datetime.datetime(
                    fitrec['time.yr'],
                    fitrec['time.mo'],
                    fitrec['time.dy'],
                    fitrec['time.hr'],
            ...
            tmp = {}
            tmp['fit_ut'] = fit_ut
            tmp['tfreq']  = fitrec['tfreq']
            fit_to_csv.append(tmp)

        fit_to_csv = pd.DataFrame(fit_to_csv)
        fit_csv_path = os.path.join(self.run_dct['output_dir'],'superdarn_fitacf.csv')
        fit_to_csv.to_csv(fit_csv_path,index=False)
        tf = np.logical_and(fit_to_csv['fit_ut'] >= sDate,fit_to_csv['fit_ut'] < eDate)
