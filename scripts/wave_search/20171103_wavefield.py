...
#        result      = data.plot.contour(x=xkey,y='dist_Km',ax=ax,vmin=vmin,vmax=vmax,levels=5,zorder=1000)
    ...
    def plot_stackplot_and_keogram(self):
        ...
        for linx,lkey in enumerate(lkeys):
            ...
            fig         = plt.figure(figsize=(np.array([35,nrows*2])*1.25))
            col_0       = 0
            col_0_span  = 30
            col_1       = 36
            col_1_span  = 65
            ...
            for ginx,rgn in enumerate(grid[::-1]):
                ...
                ax = plt.subplot2grid((nrows,ncols),(ginx,col_0),
                        projection=ccrs.PlateCarree(),colspan=col_0_span)
                self.map_ax(dft,ax)

                ################################################################################ 
                # Plot Time Series ############################################################# 
                ax      = plt.subplot2grid((nrows,ncols),(ginx,col_1),colspan=col_1_span)
            ...
            fig.text(0.5,0.89,fname,fontdict={'weight':'bold','size':36},ha='center')
